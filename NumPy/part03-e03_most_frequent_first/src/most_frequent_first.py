...
def most_frequent_first(a, c):
    ...
    column_get = a[:, c]
    values, count = np.unique(column_get, return_counts=True)

    freq_map = dict(zip(values, count))

    sorted_indices = np.argsort([freq_map[val] for val in a[:, c]])[::-1]

    return a[sorted_indices]

def main():
    np.random.seed(0)
    array = np.random.randint(1, 9, (8, 5))

    print(array)

    print()

    print(most_frequent_first(array, 2))
